...
int main() {
    ...
    // The bug lies in the fact that for a fresh board, we are checking the index of -1 first at line 30
...
namespace dsac::array {
    ...
    void Scoreboard::add(int score, const string& name) {
        ...
        if (score > board[num_entries - 1].get_score() || num_entries < capacity) {
            println("Bad index: {}",  board[num_entries - 1].get_score());
            println("Validating board[-1] is equal to above: {}", board[-1].get_score());
