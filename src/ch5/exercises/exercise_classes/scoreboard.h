...
class Scoreboard {
  ...
  public:
    ...
    /// @param int score
    /// @param string name
    virtual void add(int score, const std::string& name);
    ...
    virtual GameEntry remove(int i);
    ...
    /// destructor
    virtual ~Scoreboard() { delete[] board; }

  private:
    void clone(const Scoreboard& other);
    void transfer(Scoreboard& other);
    bool exceeds_entry_count(const std::string& name) const ;
    int get_lowest_player_score(const std::string& name) const;
