...
class CircularlyLinkedList {
  ...
  public:
    ...
    void push_back(const T& elem) {
        ...
        tail = tail->next;                            // then rotate the newest node to be tail
    }
...
size_t iterative_size() {
    ...
    bool operator==(const CircularlyLinkedList& other) const {
        if (size() != other.size()) {
            return false;
        }
        if (size() == 0) {return true;}
        if (size() == 1) {return tail->elem == other.tail->elem;}


        auto firstPtr{tail->next};
        auto secondPtr{other.tail->next};
        auto firstEnd{tail};
        auto secondEnd{other.tail};

        while (firstPtr != firstEnd) {
            if (firstPtr->elem != secondPtr->elem) {
                return false;
            }
