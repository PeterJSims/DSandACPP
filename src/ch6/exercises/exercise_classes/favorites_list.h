...
class FavoritesList {
  ...
  public:
    ...
    void reset_counts() {
        iterator ptr{contents.begin()};
        while (ptr != contents.end()) {
            (*ptr).first = 0;
            ++ptr;
        }
    }
    ...
    std::list<int> get_counts() const {
        const_iterator walk{contents.begin()};
        std::list<int> result;
        while (walk != contents.end()) {
            result.push_back(walk->first);                                // add original element to the results
            ++walk;
        }
        return result;
    }
