...
class DoublyLinkedList {
  ...
  public:
    ...
    iterator insert(iterator it, const T& elem) {
        if (!bad_check(it)) {
            throw std::invalid_argument("Iterator must belong to this class");
        }
    ...
    bool bad_check(const_iterator& other) {
  ...
  public:
    ...
    void reverse() {
        ...
        Node* original_first{header->next};
        Node* original_last{trailer->prev};
        ...
        header->next = original_last;
        original_last->prev = header;
        trailer->prev = original_first;
        original_first->next = trailer;
    }
